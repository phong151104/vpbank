...
# ----------------------------------------------------------------------------
md('''# VPINS — AIA Challenge: Feature Engineering chuyên sâu (tách riêng)
...
**Bàn giao:** `build_features_v2()` + `load_feature_set()` trong `src/features.py`; `outputs/feature_set.json` + `outputs/train_fe.parquet` / `test_fe.parquet`.
...
plt.tight_layout(); savefig("FE01_quality_audit"); plt.show()''')
...
savefig("FE02_corr_with_target"); plt.show()''')
...
plt.tight_layout(); savefig("FE03_cramers_v"); plt.show()''')
...
savefig("FE04_corr_heatmap"); plt.show()
...
full_tbl.round(5).to_csv("outputs/feature_importance_full.csv", index=False, encoding="utf-8-sig")
print(f"Đã ghi bảng FULL {len(full_tbl)} feature × {len(METHODS)} phương pháp -> outputs/feature_importance_full.csv")
...
perfK.to_csv("outputs/perf_vs_K.csv", index=False, encoding="utf-8-sig")
...
print("=> Xem outputs/feature_importance_full.csv để tự chọn, rồi báo K muốn chốt.")''')
...
plt.tight_layout(); savefig("FE06_selection_compare"); plt.show()''')
...
# ----------------------------------------------------------------------------
md('''## 9. Bàn giao artifact (feature_set.json + parquet)''')
code('''feature_set = {
...
with open("outputs/feature_set.json", "w", encoding="utf-8") as f:
...
try:
    Xtr_out.to_parquet("outputs/train_fe.parquet"); Xte_out.to_parquet("outputs/test_fe.parquet")
    fmt = "parquet"
except Exception as e:
    Xtr_out.to_csv("outputs/train_fe.csv", index=False); Xte_out.to_csv("outputs/test_fe.csv", index=False); fmt = "csv"
print(f"Đã ghi: outputs/feature_set.json + train_fe/test_fe ({fmt})")

# verify load
from src.features import load_feature_set
ld = load_feature_set("final_selected")
print(f"load_feature_set('final_selected') -> {len(ld)} cột | khớp: {ld == list(FINAL_FEATURES)}")
print("\\nCách dùng ở model_v2: COLS_X = load_feature_set('final_selected'); rồi truyền vào oof_proba/build_pipeline như cũ.")''')
...
print("\\nArtifact: outputs/feature_set.json, outputs/train_fe.parquet, test_fe.parquet")
print("Figures: figures/FE01..FE06")''')

# ----------------------------------------------------------------------------
md('''## 11. Feature importance — TOÀN BỘ feature (xem độ chênh)

Vẽ consensus của cả 125 feature (thanh lỗi = `rank_std` = độ bất nhất giữa 7 phương pháp), tô màu theo nhóm đặc trưng. Cell **tự chứa** (đọc `outputs/feature_importance_full.csv`) nên chạy độc lập được.''')
code('''# Cell tự chứa: đọc outputs/feature_importance_full.csv -> vẽ full importance
import pandas as pd, numpy as np, matplotlib.pyplot as plt
import matplotlib.patches as mpatches
plt.rcParams["font.sans-serif"] = ["Segoe UI", "Arial", "Tahoma", "DejaVu Sans"]
plt.rcParams["axes.unicode_minus"] = False

tbl = pd.read_csv("outputs/feature_importance_full.csv"); tbl["col"] = tbl["col"].astype(str)
def _grp(c):
...
plt.tight_layout(); plt.savefig("figures/FE09_all_importance.png", dpi=150, bbox_inches="tight"); plt.show()
...
with open("feature_engineering.ipynb", "w", encoding="utf-8") as f:
    nbf.write(nb, f)
print("WROTE feature_engineering.ipynb with", len(cells), "cells")
