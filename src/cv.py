...
def oof_proba(spec, X, y, cols, n_splits=5, n_repeats=3, seed=SEED, n_jobs=1):
    ...
    for r in range(n_repeats):
        ...
        p = cross_val_predict(pipe, X, y, cv=skf, method="predict_proba", n_jobs=n_jobs)[:, 1]
        oof += p
...
def run_zoo(models, X, y, cols, n_splits=5, n_repeats=3, seed=SEED, verbose=True):
    """Chạy toàn bộ model zoo, trả (bảng kết quả, dict OOF proba)."""
    rows, oof_dict = [], {}
    for name, spec in models.items():
        oof, per_rep = oof_proba(spec, X, y, cols, n_splits, n_repeats, seed)
        oof_dict[name] = oof
        s = summarize(y, oof)
