...
from .metrics import topk_hits

optuna.logging.set_verbosity(optuna.logging.WARNING)
SEED = 42


def _cv_oof(make_model, fit_kwargs_fn, X, y, cols, n_splits, seed):
...
def _objective_factory(name, X, y, cols, spw, n_splits, seed):
    def objective(trial):
        ...
        oof, _ = _cv_oof(mk, fk, X, y, cols, n_splits, seed)
...
def _best_iter_for(name, bp, X, y, cols, spw, n_splits, seed):
    if name == "LightGBM":
        ...
        fk = lambda Xv, yv: dict(eval_set=(Xv, yv), early_stopping_rounds=60, verbose=0)
    _, mi = _cv_oof(mk, fk, X, y, cols, n_splits, seed)
    return mi
